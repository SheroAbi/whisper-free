...
class ParakeetASR:
    ...
    def transcribe_tokens(self, audio_f32: np.ndarray) -> tuple[list[tuple[str, float, float]], float]:
        """Decode with per-token times: [(piece, start_s, end_s), ...] + elapsed ms.

        Uses the TDT model's native token timing (free with the decoder) so the
        engine can stitch partial results across a sliding window. Supported by
        onnx-asr models exposing with_timestamps(); engine.py falls back to
        plain transcribe() otherwise.
        """
        # Fast path: the raw batch API (via the loader's unwrapped model) with
        # no `need_logprobs` - the public with_timestamps() adapter forces a
        # per-step log_softmax over the whole vocab that partials never use.
        raw = getattr(self.model, "asr", None) or self.model
        t0 = time.perf_counter()
        try:
            result = next(raw.recognize_batch(
                audio_f32[np.newaxis, :],
                np.array([audio_f32.size], dtype=np.int64),
            ))
            pieces = list(getattr(result, "tokens", None) or [])
            stamps = list(getattr(result, "timestamps", None) or [])
        except Exception:
            return self._transcribe_tokens_adapter(audio_f32)
        elapsed = (time.perf_counter() - t0) * 1000.0

        out: list[tuple[str, float, float]] = []
        dur = audio_f32.size / 16000.0  # Parakeet ONNX runs at 16 kHz
        for i, piece in enumerate(pieces):
            start = float(stamps[i]) if i < len(stamps) else 0.0
            end = float(stamps[i + 1]) if i + 1 < len(stamps) else dur
            out.append((str(piece), start, end))
        return out, elapsed

    def _transcribe_tokens_adapter(self, audio_f32: np.ndarray) -> tuple[list[tuple[str, float, float]], float]:
        """Fallback: public with_timestamps() adapter (slower, always works)."""
        assert self.model is not None
        if getattr(self, "_ts_model", None) is None:
            self._ts_model = self.model.with_timestamps()

        t0 = time.perf_counter()
        result = self._ts_model.recognize(audio_f32)
        elapsed = (time.perf_counter() - t0) * 1000.0
